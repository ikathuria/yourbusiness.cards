...
| 7. AI generation (Premium) | ◐ built | **AI card gen** (`features/generate/` + `/dashboard/generate` → Claude selects template/accent/copy from the registry via a tool-constrained schema, creates a draft, opens the editor) + **AI QR art** (`qr-art.ts` via Replicate → Supabase Storage `qr-art` bucket, `business_cards.qr_art_url`, editor panel). Gated to `premium`. Builds + typechecks; needs `ANTHROPIC_API_KEY` / `REPLICATE_API_TOKEN` set to runtime-verify. |
